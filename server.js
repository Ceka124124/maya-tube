...
/* ══ STARMAKER PROXY ══ */
app.get('/api/starmaker', async (req, res) => {
  try {
    const r = await fetch('https://m.starmakerstudios.com/go-v1/year-end-2024-friendship/87930/intimacy-tree', {
      method: 'GET',
...
wss.on('connection', ws => {
  ...
  ws.on('message', async raw => {
    ...
    switch(d.type) {
      ...
      case 'set_frame': {
        const u = store.users[ws.wsId]; if (!u) return;
        const DEFAULT_FRAME_ID = '__default_frame__';
        if (d.frameId === DEFAULT_FRAME_ID) {
          u.frame = DEFAULT_FRAME_ID;
        } else {
      ...
      /* ── ADMIN KULLANICI PROFİL ZORLA GÜNCELLE ── */
      case 'admin_force_profile': {
        const u = store.users[ws.wsId]; if (!isAdmin(u)) return;
        const t = allUsers().find(x => x.id === d.targetId); if (!t) return;
        const DEFAULT_FRAME_ID = '__default_frame__';

        // İsim güncelle
        if (d.username && d.username.trim() && d.username !== t.username) {
          const oldName = t.username;
          t.username = d.username.trim().slice(0, 22);
          // Firebase profili yeni isimle aktar
          saveUserProfile(t.username, { tur: t.tur, frame: t.frame, entranceId: t.entranceId });
        }

        // Avatar güncelle
        if (d.avatar !== undefined) {
          t.avatar = (d.avatar || '').slice(0, 500000);
          if (t.avatar) fbSet(`avatars/${encodeURIComponent(t.username)}`, t.avatar).catch(()=>{});
        }

        // Çerçeve güncelle
        if (d.frame !== undefined) {
          if (d.frame === DEFAULT_FRAME_ID) {
            t.frame = DEFAULT_FRAME_ID;
          } else {
            const fi = store.items.find(it => it.type === 'frame' && it.id === d.frame);
            t.frame = fi ? fi.id : '';
          }
          saveUserProfile(t.username, { frame: t.frame });
        }

        // Giriş efekti güncelle
        if (d.entranceId !== undefined) {
          const ei = store.items.find(it => it.type === 'entrance' && it.id === d.entranceId);
          t.entranceId = ei ? ei.id : '';
          saveUserProfile(t.username, { entranceId: t.entranceId });
        }

        // Hedef kullanıcıya force_profile gönder
        sendTo(t.id, {
          type: 'force_profile',
          targetId: t.id,
          username: t.username,
        ...
        // Herkese güncel users listesini yayınla
        bcast({ type:'users', users: allUsers() });
        bcast({ type:'chat', msg: sysMsg(`👑 Admin ${t.username} profilini güncelledi`) });
        break;
      }
