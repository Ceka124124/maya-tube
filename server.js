...
function chatMsg(u,text){const m={id:uuid(),username:u.username,tur:u.tur,text:text.slice(0,500),msgType:'text',ts:Date.now(),avatar:u.avatar||'',frame:u.frame||''};store.chat.push(m);if(store.chat.length>200)store.chat.shift();fbPush('chat',m);return m;}
  ...
  ws.on('message', raw => {
    ...
    switch(d.type){
      ...
      case 'join': {
        ...
        store.users[ws.wsId]={id:ws.wsId,username,tur:tur||'user',avatar,seat:0,muted:false,banned:false,voice:false,exp:prev?.exp||0,lvl:prev?Math.floor((prev.exp||0)/20)+1:1,frame:d.frame||'',ownedItems:[]};
      ...
      case 'chat': {
        ...
        bcast({type:'chat',msg:chatMsg(u,d.text)}); break;
      }
